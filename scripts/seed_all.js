/**
 * ============================================================
 * Kurickal TMS — Full Database Seed Script
 * ============================================================
 *
 * Creates all roles, users, projects, tasks, milestones,
 * chat channels, notifications and site diary entries.
 *
 * USAGE:
 *   node scripts/seed_all.js
 *
 * PREREQUISITES:
 *   1. Firestore must be in TEST MODE for this script to run
 *      without admin credentials. Go to Firebase Console →
 *      Firestore → Rules → temporarily set:
 *        allow read, write: if true;
 *      Then restore your rules after seeding.
 *
 *   2. Email/Password sign-in must be enabled in Firebase
 ...
 *  ---------------------|-------------------|------------------------------------|---------------
 *  Director / Owner     | Thomas Kurickal   | thomas@kurickaldevelopers.com      | Kurickal@2024
 *  Project Manager      | Ravi Nair         | ravi@kurickaldevelopers.com        | Kurickal@2024
 *  Site Engineer        | Arjun Menon       | arjun@kurickaldevelopers.com       | Kurickal@2024
 *  Site Engineer        | Priya Pillai      | priya@kurickaldevelopers.com       | Kurickal@2024
 *  Foreman              | Suresh Kumar      | suresh@kurickaldevelopers.com      | Kurickal@2024
 *  Labour               | Biju Thomas       | biju@kurickaldevelopers.com        | Kurickal@2024
 *  Admin                | Meena George      | meena@kurickaldevelopers.com       | Kurickal@2024
 *  Accounts             | Anitha Raj        | anitha@kurickaldevelopers.com      | Kurickal@2024
 *
 * ============================================================
 */
...
function ts(date) {
  return { timestampValue: (date instanceof Date ? date : new Date(date)).toISOString() };
}
...
async function firestoreSet(collection, docId, fields, idToken) {
  ...
  if (!res.ok) {
    ...
    console.error(`  ✗ ${collection}/${docId}: ${res.status} ${text.substring(0, 200)}`);
  }
  return res;
}
...
async function firestoreAdd(collection, fields, idToken) {
  ...
  if (!res.ok) {
    const text = await res.text();
    console.error(`  ✗ POST ${collection}: ${res.status} ${text.substring(0, 200)}`);
    return null;
  }
  const data = await res.json();
  // Extract doc id from name: "projects/{id}/databases/(default)/documents/{col}/{docId}"
...
async function createAuthUser(email, password, displayName) {
  // Try sign up first
  let res = await fetch(`${AUTH_BASE}/accounts:signUp?key=${API_KEY}`, {
    method: "POST",
  ...
  if (data.error) {
    if (data.error.message === "EMAIL_EXISTS") {
      // Sign in instead
      res = await fetch(`${AUTH_BASE}/accounts:signInWithPassword?key=${API_KEY}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      data = await res.json();
      if (data.error) {
        console.error(`  ✗ Auth sign-in failed for ${email}:`, data.error.message);
        return null;
      }
      console.log(`  ↩ Already exists, signed in: ${email}`);
    } else {
      console.error(`  ✗ Auth sign-up failed for ${email}:`, data.error.message);
      return null;
    }
  } else {
...
async function main() {
  console.log("\n╔══════════════════════════════════════════════════╗");
  console.log("║     Kurickal TMS — Database Seed Script          ║");
  console.log("╚══════════════════════════════════════════════════╝\n");

  // ── 1. Roles ────────────────────────────────────────────────────────────────
  console.log("📋 Seeding roles...");
  for (const role of ROLES) {
    await firestoreSet("roles", role.id, {
      name: str(role.name),
  ...
  for (const user of USERS_DEF) {
    const result = await createAuthUser(user.email, PASSWORD, user.name);
    if (!result) continue;
    const { uid, idToken } = result;
    uids[user.name] = uid;

    if (user.roleId === "director") directorToken = idToken;
    ...
    console.log(`  ✓ Firestore doc: ${user.name} [${user.roleId}]`);
  }

  if (!directorToken) {
    console.error("\n✗ Could not get director token. Aborting project/task seeding.");
    return;
  }
  ...
  const proj1Id = await firestoreAdd("projects", {
  ...
  const proj2Id = await firestoreAdd("projects", {
  ...
  const proj3Id = await firestoreAdd("projects", {
  ...
  for (const [uid, projIds] of Object.entries(userProjectMap)) {
    await firestoreSet("users", uid, { projectIds: strArr(projIds) });
  }
  ...
  for (const m of proj1Milestones) {
    ...
    await firestoreAdd(`projects/${proj1Id}/milestones`, fields);
  }
  ...
  for (const m of proj2Milestones) {
    ...
    await firestoreAdd(`projects/${proj2Id}/milestones`, fields);
  }
  ...
  for (const t of tasks) {
    const id = await firestoreAdd("tasks", {
      title:          str(t.title),
  ...
  if (firstTaskId) {
    ...
    for (const sub of subtasks) {
      await firestoreAdd(`tasks/${firstTaskId}/subtasks`, {
        title:  str(sub.title),
    ...
    // Add a comment
    await firestoreAdd(`tasks/${firstTaskId}/comments`, {
  ...
  function buildUnreadCounts(members, senderUid, count = 1) {
    ...
    for (const uid of members) {
      fields[uid] = int(uid === senderUid ? 0 : count);
    }
    return map(fields);
  }

  // Channel 1: Company Announcements
  await firestoreSet("chats", "ch_announcements", {
    ...
    lastMessageText: str("Welcome to Kurickal TMS! This is your central hub for all project communication."),
    lastMessageAt:   ts(daysAgo(60)),
    lastMessageBy:   str(thomasUid),
    isArchived:      bool(false),
    unreadCounts:    buildUnreadCounts(allUids, thomasUid, 1),
  });
  // Seed the announcement message
  await firestoreAdd("chats/ch_announcements/messages", {
    channelId:          str("ch_announcements"),
    senderId:           str(thomasUid),
    text:               str("Welcome to Kurickal TMS! 🎉 This is your central hub for all project communication. Please use the respective project channels for site updates. Direct messages are available for 1-on-1 conversations. Let's build great things together! — Thomas Kurickal, Director"),
    type:               str("text"),
    attachmentUrl:      nul(),
    attachmentName:     nul(),
  ...
  await firestoreSet("chats", `proj_${proj1Id}`, {
    type:            str("project"),
  ...
  for (const msg of [
    ...
    await firestoreAdd(`chats/proj_${proj1Id}/messages`, {
  ...
  await firestoreSet("chats", `proj_${proj2Id}`, {
    type:            str("project"),
    name:            str("Commercial Complex Edappally"),
    ...
    createdBy:       str(raviUid),
    createdAt:       ts(daysAgo(90)),
    lastMessageText: str("Column concreting starts tomorrow. Ensure vibrator is ready."),
  ...
  for (const msg of [
    ...
    { sender: arjunUid, text: "Column concreting starts tomorrow. Ensure vibrator is ready.", daysAgo_: 1 },
  ]) {
    await firestoreAdd(`chats/proj_${proj2Id}/messages`, {
  ...
  await firestoreSet("chats", "ch_site_engineers", {
    type:            str("group"),
  ...
  await firestoreAdd("chats/ch_site_engineers/messages", {
  ...
  for (const msg of [
    ...
    await firestoreAdd(`chats/${dmId}/messages`, {
  ...
  console.log("╠══════════════════════════════════════════════════╣");
  console.log("║  Roles     : 7 created                          ║");
  console.log("║  Users     : 8 created                          ║");
  console.log("║  Projects  : 3 created                          ║");
  console.log("║  Tasks     : 13 created                         ║");
  console.log("║  Channels  : 5 created                          ║");
